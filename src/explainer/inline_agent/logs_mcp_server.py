...
def _get_logs(log_channel: str) -> str:
    """
    Fetch logs for a given log channel.
    :param log_channel: the log channel to fetch logs for
    :return: logs for the given log channel
    """
    print(f"Fetching logs for log_channel: {log_channel}")
    if log_channel == "abc":
        return """
...
2025-04-06T21:34:22-04:00 TypeError: '>=' not supported between instances of 'DatetimeProperties' and 'int'    
    """
    return "Logs not found for this log_channel"

@mcp.tool()
def get_logs_for_log_channel(log_channel: str) -> str:
    return _get_logs(log_channel)
